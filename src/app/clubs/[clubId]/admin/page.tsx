/**
 * Admin dashboard — URL-based route (/clubs/[clubId]/admin)
 *
 * PERFORMANCE: getCachedSession + getCachedMembership son zero-cost aquí
 * porque ClubLayout y AdminGuard ya los llamaron en el mismo render pass.
 * Las 13 queries de negocio corren en paralelo con Promise.all.
 *
 * Antes (re-export): 3×getServerSession + 3×membership query + 13 queries de negocio
 * Ahora: 0 queries extra de auth + 13 queries de negocio en paralelo
 */
...
import { getCachedSession, getCachedMembership } from '@/lib/session'
import {
  Users, Wallet, ShoppingBag, Vote,
  TrendingUp, TrendingDown, Clock, Calendar,
  AlertCircle, FileText, ClipboardList,
} from 'lucide-react'
...
export default async function AdminDashboard({
  ...
  // Zero-cost: cached by ClubLayout + AdminGuard in this same request
  ...
  const [
    ...
    prisma.clubEvent.findMany({
      where: { clubId, startAt: { gte: new Date() } },
      orderBy: { startAt: 'asc' },
      take: 3,
      ...
      <main className="flex-1 p-6 space-y-6">
        ...
        <div className="grid grid-cols-4 gap-4">
          <StatCard
            ...
            color="blue"
          />
          <StatCard title="Saldo bancario" value={fmtCurrency(balance)} icon={Wallet} color="green" />
          <StatCard title="Campañas abiertas" value={openWindows} icon={ShoppingBag} color="orange" />
        ...
        {(pendingMembersCount > 0 || pendingInvoices > 0 || openTickets > 0 || pendingOrders > 0) && (
          <div className="flex gap-3 flex-wrap">
            {pendingMembersCount > 0 && (
              <a href={`${base}/members?status=PENDING`}
            ...
            {pendingInvoices > 0 && (
              <a href={`${base}/accounting?tab=invoices`}
        ...
        <Card>
          <CardHeader>
            ...
            <a href={`${base}/accounting`} className="text-sm text-primary hover:underline">Ver todos</a>
          </CardHeader>
        ...
        {pendingMembersList.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <a href={`${base}/members`} className="text-sm text-primary hover:underline">Ver todos</a>
            </CardHeader>
            <div className="space-y-0 divide-y divide-gray-50">
        ...
        {upcomingEvents.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <a href={`${base}/events`} className="text-sm text-primary hover:underline">Ver todos</a>
            </CardHeader>
            <div className="space-y-0 divide-y divide-gray-50">
