...
export default function AnnouncementsPage() {
  ...
  const { data, mutate } = useSWR<any>(
    `/api/clubs/${clubId}/announcements?page=${page}`,
    { keepPreviousData: true }
  )

  // Fetch events for segmentation selector in parallel — no dependency on clubId loading
  const { data: eventsData } = useSWR<any>(`/api/clubs/${clubId}/events?pageSize=50`)
  ...
  const remove = async (id: string) => {
    ...
    if (res.ok) { toast.success('Anuncio eliminado'); mutate() }
  }
  ...
  return (
    <div className="flex flex-col flex-1 overflow-auto">
      <Header title="Anuncios y Comunicados" />
